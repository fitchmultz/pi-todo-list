...
The tool definition and system-prompt guidance are static. Mutations return only the change and status counts; the full state stays in persisted tool-result details. After compaction, the extension injects one compact active/pending snapshot when the retry or next agent turn starts, so later mutations cannot stale it and the provider-cacheable conversation prefix stays intact.
